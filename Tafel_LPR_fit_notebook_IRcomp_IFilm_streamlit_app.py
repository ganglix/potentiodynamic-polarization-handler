...
class Tafit:
    ...
    def BV_LPR_interact_streamlit(self, anodic_range=0.15, cathodic_range=0.15):
        """
    ...
    def BVF_LPR_manual(
        ...
        popt, pcov = curve_fit(
            BVFeq, E_select, I_select, p_guess, bounds=bound
        )  # popt is optimal parameter array
        ############## end of Fitting parameters from scan#############
        ...
        if logx:
            ...
            self.line_bvf.set_data(
                np.abs(BVFeq(_, *popt)), _
            )  # BVF Fitted from observation
            self.ax.semilogx()
        ...
        print((
            "Goodness of fit, R2_score:",
            r2_score(I_select, BVFeq(E_select, *popt)),
            "Chi squared:",
            np.sum((I_select - BVFeq(E_select, *popt)) ** 2 / BVFeq(E_select, *popt)),
        ))
    ...
    def BVF_LPR_interact(self, anodic_range=0.15, cathodic_range=0.15):
        ...
        widgets.interact(
            ...
            auto_zoom=widgets.Checkbox(
                value=True, description="Auto_zoom", disabled=False
            ),
    ...
    def remove_outlier(self, roughness=10):
        ...
        try:
            out_lier_idx = self.data.index[0:-1][
                np.abs(np.diff(self.data["i_density"]))
    ...
    def remove_outlier_manual(self, data_range):
        out_lier_idx = list(range(data_range[0], data_range[1]))
        self.data.iloc[self.data.iloc[out_lier_idx].index] = np.nan
        self.data.dropna(axis="index", inplace=True)
        return self
    ...
    def plot_BV_F_components(self, logx=True):
        ...
        if logx:
            plt.plot(
                BVFeq(self.data.E, *self.result.values[0:6]).abs(),
                self.data.E,
                label="BVF",
            )
            ...
            plt.plot(
                BVFeq(self.data.E, *self.result.values[0:6]), self.data.E, label="BVF"
            )
            ...
            plt.plot(
                BVFeq(self.data.E, *self.result.values[0:6])
                - BVeq(self.data.E, *self.result.values[0:4]),
                self.data.E,
                label="F_component",
            )
...
def plot_compare(
    info_obj_list, offset_drift=True, logx=True, absolute_value=True, inplace=False
):
    """
    ...
    for this_obj, this_drift in zip(info_obj_list, drifts):
        ...
        if absolute_value:
            this_obj.get_data().plot(
                ...
                markersize=1,
                logx=logx,
                label=this_obj.get_filename().split("/")[-1].split(".xlsx")[0],
            )
        else:
            this_obj.get_data().plot(
                x="i_density",
